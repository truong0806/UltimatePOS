$(document).ready(function () {
    ...
    supplier_report_tbl = $('#supplier_report_tbl').DataTable({
        ...
        columns: [
            ...
            { data: 'total_construction_invoice', name: 'total_construction_invoice' },
        ...
        fnDrawCallback: function (oSettings) {
            ...
            var total_construction_sell = sum_table_col(
                $('#supplier_report_tbl'),
                'total_construction_invoice'
            );
            $('#footer_total_construction_sell').text(total_construction_sell);
    ...
    construction_report_tbl = $('#construction_report_tbl').DataTable({
        processing: true,
        serverSide: true,
        ajax: {
            url: '/reports/construction',
            data: function (d) {
                d.customer_group_id = $('#cnt_customer_group_id').val();
                d.contact_type = $('#contact_type').val();
                d.location_id = $('#cs_report_location_id').val();
        ...
        columns: [
            { data: 'name', name: 'name' },
            {
                ...
                render: function (data, type, row) {
                    if (row.contact_name) {
                        var url = '/UltimatePOS/public/contacts/' + row.contact_id;
        ...
        fnDrawCallback: function (oSettings) {
            var total_purchase = sum_table_col($('#construction_report_tbl'), 'total_purchase');
            $('#footer_total_purchase').text(total_purchase);

            var total_purchase_return = sum_table_col(
                $('#construction_report_tbl'),
                'total_purchase_return'
            );
            $('#footer_total_purchase_return').text(total_purchase_return);

            var total_sell = sum_table_col($('#construction_report_tbl'), 'total_invoice');
            $('#footer_total_sell').text(total_sell);
            ...
            var total_due = sum_table_col($('#construction_report_tbl'), 'total_due');
            $('#footer_total_due').text(total_due);

            __currency_convert_recursively($('#construction_report_tbl'));
        },
    });
    ...
    sell_payment_report = $('table#sell_payment_report_table').DataTable({
        ...
        columns: [
            ...
            { data: 'amount', name: 'transaction_payments.amount' },
            {
                ...
                render: function (data, type, row) {
                    if (row.advance_balance) {
                        return (
                            '<span class="advanced-amount" data-orig-value="' +
                            data +
                            '">' +
                            data +
                            '</span>'
            ...
            { data: 'customer_group', name: 'customer_group', searchable: false },
            {
                ...
                searchable: false,
                render: function (data, type, row) {
                    if (row.construction_payment) {
                        var url = '/UltimatePOS/public/constructions/' + row.construction_payment;
                        return '<a target="_blank" href="' + url + '">' + data + '</a>';
                    }
                    return data;
                },
        ...
        fnDrawCallback: function (oSettings) {
            ...
            var total_advanced_amount = 0;
            $('#sell_payment_report_table tbody tr').each(function () {
                var advancedAmountText = $(this)
                    .find('td:eq(4) .advanced-amount')
                    .data('orig-value');
                if (advancedAmountText) {
                    console.log('🚀 ~ advancedAmountText:', advancedAmountText);
                    var numericValue = parseFloat(advancedAmountText.replace(/[₫,]/g, ''));
                    total_advanced_amount += isNaN(numericValue) ? 0 : numericValue;
                    console.log('🚀 ~ total_advanced_amount:', total_advanced_amount);
                }
            });

            var formatted_total_advanced_amount = formatCurrency(total_advanced_amount);
            console.log('🚀 ~ formatted_total_advanced_amount:', formatted_total_advanced_amount);

            $('#footer_total_amount').text(formatCurrency(total_amount));

            $('#footer_total_overpayment').text(formatted_total_advanced_amount);
