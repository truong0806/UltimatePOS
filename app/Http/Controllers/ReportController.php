...
class ReportController extends Controller
{
        ...
        if ($request->ajax()) {
            $contacts = Contact::where('contacts.business_id', $business_id)
                ...
                ->select(
                    ...
                    DB::raw("SUM(IF(t.type = 'sell' AND t.status = 'final' AND t.construction_id IS NOT NULL, final_total, 0)) as total_construction_invoice"),
            ...
            return Datatables::of($contacts)
                ...
                ->editColumn(
                    ...
                    '<span class="total_construction_invoice" data-orig-value="{{$total_construction_invoice}}">@format_currency($total_construction_invoice)</span>'
                )
                ...
                ->rawColumns(['total_purchase', 'total_invoice', 'total_construction_invoice', 'due', 'name', 'total_purchase_return', 'total_sell_return', 'opening_balance_due'])
                ->make(true);
        }
    ...
    public function getConstructionReport(Request $request)
    {
        if (! auth()->user()->can('contacts_report.view')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        //Return the details in ajax call
        if ($request->ajax()) {
            $construction = Construction::join('transactions AS t', 'constructions.id', '=', 't.construction_id')
                ->leftJoin(
                    'contacts AS cs',
            ...
            if ($permitted_locations != 'all') {
                $construction->whereIn('t.location_id', $permitted_locations);
            }


            if (! empty($request->input('location_id'))) {
                $construction->where('t.location_id', $request->input('location_id'));
            }
            ...
            $end_date = $request->get('end_date');
            if (! empty($start_date) && ! empty($end_date)) {
                $construction->where('t.transaction_date', '>=', $start_date)
                    ->where('t.transaction_date', '<=', $end_date);
            }

            return Datatables::of($construction)
                ->editColumn('name', function ($row) {
                    $name = $row->name;
                    return '<a href="' . action([\App\Http\Controllers\ConstructionController::class, 'show'], [$row->id]) . '" target="_blank" class="no-print">' .
                        $name .
                        '</a>';
                })
                ...
                ->filterColumn('name', function ($query, $keyword) {
                    $query->where(function ($q) use ($keyword) {
                        $q->where('construction.name', 'like', "%{$keyword}%");
                    });
                })
                ->rawColumns(['total_purchase', 'total_invoice', 'total_construction_invoice', 'due', 'name', 'total_purchase_return', 'total_sell_return', 'opening_balance_due'])
                ->make(true);
    ...
    public function sellPaymentReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($query)
                ...
                ->editColumn('constructions_name', function ($row) {
                    if (!empty($row->construction_payment)) {
                        return $row->constructions_name . ' - ' . $row->construction_payment;
                    } else {
                        return '';
                    }
                })
                ...
                ->editColumn('advance_balance', function ($row) {
                    return $this->transactionUtil->num_f($row->advance_balance, true);
                })
                ->addColumn('action', '<button type="button" class="btn btn-primary btn-xs view_payment" data-href="{{ action([\App\Http\Controllers\TransactionPaymentController::class, \'viewPayment\'], [$DT_RowId]) }}">@lang("messages.view")
                    </button> @if(!empty($document))<a href="{{asset("/uploads/documents/" . $document)}}" class="btn btn-success btn-xs" download=""><i class="fa fa-download"></i> @lang("purchase.download_document")</a>@endif')
