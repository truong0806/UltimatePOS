    ...
    <section class="content">
            ...
            <div class="col-md-12 col-sm-12">
                @component('components.widget', ['class' => 'box-solid'])
                    ...
                    <div class="@if (!empty($commission_agent)) col-sm-3 @else col-sm-4 @endif">
                        ...
                        <div class="form-group">
                            {!! Form::label('contact_id', __('messages.constructions') . ':*') !!}
                            <div class="input-group">
                                <span class="input-group-addon">
                                    <i class="fa fa-building"></i>
                                </span>
                                <input type="hidden" id="default_construction_id" value="{{ $transaction->construction_id }}">
                                <input type="hidden" id="default_construction_name"
                                    value="{{ $transaction->construction_name ?? '' }}">
                                {!! Form::select('construction_id', $construction, $transaction->construction_id ?? null, [
                                    'class' => 'form-control select2',
                                    'id' => 'construction_id',
                                    'placeholder' => __('messages.please_select'),
                                ]) !!}
                                <span class="input-group-btn">
                                    <button type="button" class="btn btn-default bg-white btn-flat add_new_construction"
                                        data-name="">
                        ...
                        @include('sale_pos.partials.construction_create_modal')
        ...
        <div aria-live="polite" aria-atomic="true" style="position: relative; min-height: 200px;">
            <div style="position: absolute; top: 0; right: 0;">
                ...
                <!-- Toast -->
                <div class="toast" id="constructionToast" role="alert" aria-live="assertive" aria-atomic="true"
                    data-delay="5000">
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            $('#construction_id').select2({
                placeholder: $('#construction_id').data('placeholder'),
                allowClear: true
            });


            $('.add_new_construction').click(function() {
                $('#createConstructionModal').modal('show');
            });
            ...
            $('#saveConstructionButton').on('click', function(event) {
                ...
                var startDate = $('#construction_start_date').val();
                var endDate = $('#construction_end_date').val();
                var budget = $('#construction_budget').val();
                if (!name) {
                ...
                var payload = {
                    ...
                    description: $('#construction_description').val(),
                    start_date: $('#construction_start_date').val(),
                    end_date: $('#construction_end_date').val(),
                    budget: $('#construction_budget').val()
                };
                ...
                if (!validateForm(form)) {
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            if (response.success) {
                                var newConstruction = response.data;
                                var newOption = new Option(newConstruction.name, newConstruction
                                    .id, false, false);
                                $('#construction_id').append(newOption).trigger('change');
                                $('#construction_id').val(newConstruction.id).trigger('change');

                                $('#createConstructionModal').modal('hide');

                                alert('Construction created successfully');
                                $('#create_construction_form')[0].reset();
                            } else {
